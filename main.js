...
;
let studants = [{ name: "urooba" }, { name: "siddiqui" }];
let teachers = [
    { name: "Miss FATIMA", experience: 3, school: " Sindh public" },
];
let courses = ["Digital Marketing", "Artificial intelligence"];
let newStudants = []; // array to store new studants.
async function main(studant) {
    do {
        ...
        if (check.check2 === "Log In") {
            ...
            const isNameValidExisting = studant.some((studant) => studant.name === enteredName);
            const isNameValidNew = newStudants.some((studant) => studant.name === enteredName);
        ...
        if (check.ckeck2 === "creat new studant") {
            ...
            const newStudantobject = { name: newStudant.newStudantName };
